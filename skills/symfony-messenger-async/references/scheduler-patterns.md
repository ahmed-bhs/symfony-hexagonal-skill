...
use App\Cleanup\Application\UseCase\Command\PurgeExpiredSessions\PurgeExpiredSessionsCommand;
use App\Report\Application\UseCase\Command\GenerateDailyReport\GenerateDailyReportCommand;
...
final class AppScheduleProvider implements ScheduleProviderInterface
{
    public function getSchedule(): Schedule
    {
        ...
        $schedule->add(
            RecurringMessage::every('1 hour', new PurgeExpiredSessionsCommand())
        );

        // Daily at 6 AM: generate report
        $schedule->add(
            RecurringMessage::cron('0 6 * * *', new GenerateDailyReportCommand())
        );

        // Every 5 minutes: process pending webhooks
        $schedule->add(
            RecurringMessage::every('5 minutes', new ProcessPendingWebhooksCommand())
        );
...
```php
namespace App\Cleanup\Application\UseCase\Command\PurgeExpiredSessions;

final readonly class PurgeExpiredSessionsCommand
{
...
final readonly class PurgeExpiredSessionsHandler implements CommandHandlerInterface
{
    public function __construct(
    ...
    public function __invoke(PurgeExpiredSessionsCommand $command): void
    {
...
final class MaintenanceScheduleProvider implements ScheduleProviderInterface
{
    ...
    {
        $schedule = new Schedule();
        $schedule->add(RecurringMessage::cron('0 2 * * *', new DatabaseBackupCommand()));
        $schedule->add(RecurringMessage::cron('0 3 * * 0', new LogRotationCommand()));
        return $schedule;
    }
}
...
final class ReportsScheduleProvider implements ScheduleProviderInterface
{
    ...
    {
        $schedule = new Schedule();
        $schedule->add(RecurringMessage::cron('0 6 * * *', new GenerateDailyReportCommand()));
        $schedule->add(RecurringMessage::cron('0 7 * * 1', new GenerateWeeklyReportCommand()));
        return $schedule;
    }
}
```
