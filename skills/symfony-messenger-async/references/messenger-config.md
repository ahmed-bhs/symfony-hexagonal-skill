...
framework:
    messenger:
        ...
        routing:
            # High priority
            'App\Payment\Application\UseCase\Command\ProcessPayment\ProcessPaymentCommand': async_priority_high
            'App\Order\Application\UseCase\Command\ConfirmOrder\ConfirmOrderCommand': async_priority_high

            # Normal priority
            'App\Report\Application\UseCase\Command\GenerateReport\GenerateReportCommand': async
            'App\Email\Application\UseCase\Command\SendEmail\SendEmailCommand': async

            # Low priority
            'App\Analytics\Application\UseCase\Command\TrackEvent\TrackEventCommand': async_priority_low
            'App\Cleanup\Application\UseCase\Command\PurgeExpiredSessions\PurgeExpiredSessionsCommand': async_priority_low
```
    ...
    messenger:
        buses:
            command.bus:
                middleware:
                    - validation
                    - App\Shared\Infrastructure\Middleware\AuditLogMiddleware
                    - doctrine_transaction
```
