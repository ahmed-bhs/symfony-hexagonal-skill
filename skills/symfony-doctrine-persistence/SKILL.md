...
1. **Mapping allowed two ways**: `#[ORM\...]` attributes on the Domain entity (inert metadata, entity stays plain-PHP testable) OR externalised XML — pick one per project. Doctrine imposes the same runtime constraints either way, so attributes are the pragmatic default.
2. **DB-access validators stay out of Domain**: `#[UniqueEntity]`, `#[Assert\Callback]`, `#[Assert\Expression]` run queries/services → put them in `Adapter/In/Validation/`. Format constraints (`#[Assert\Email]`, `#[Assert\Length]`) are fine as inert attributes.
3. **External XML mapping** (if chosen) lives in `Infrastructure/Adapter/Out/Doctrine/Mapping/`
...
use App\{Module}\Domain\Port\Out\{Entity}RepositoryInterface;
...
<doctrine-mapping>
    <entity name="App\User\Domain\Entity\User" table="users">
        ...
        <embedded name="email" class="App\User\Domain\ValueObject\Email" />
