...
cp -r /tmp/hex-skill/commands/* your-project/.claude/commands/
...
You can commit the `.claude/skills/`, `.claude/agents/`, and `.claude/commands/` directories so the entire team shares the same rules, or add them to `.gitignore` to keep it local.
...
ln -s ~/symfony-hexagonal-skill/commands .claude/commands
...
Module-first: each bounded context owns its three layers. Entry points are
driving adapters — there is no separate Presentation layer.

```
src/{Module}/
├── Domain/
│   ├── Entity/            # Aggregate roots, entities (plain PHP; #[ORM]/#[Assert] attributes allowed)
│   ├── ValueObject/       # Immutable, self-validating value objects
│   ├── Event/             # Domain events (past-tense naming)
│   ├── Exception/         # Domain-specific exceptions
│   └── Port/Out/          # Driven ports the domain needs (repositories…)
│
├── Application/
│   ├── UseCase/Command/{Name}/  # {Name}Command + {Name}Handler colocated
│   ├── UseCase/Query/{Name}/    # {Name}Query + {Name}Handler colocated
│   ├── Port/Out/          # Technical driven ports (storage, read models)
...
The command/query/event buses are driving ports in `Shared/Application/Port/In/`.

Tests mirror this structure module-first:

```
tests/
├── Unit/{Module}/Domain/                  # Pure PHP tests, no framework
├── Unit/{Module}/Application/             # Handler tests with mocked ports
├── Integration/{Module}/Infrastructure/   # Adapter ⇄ port contract, real DB
├── Functional/{Module}/                   # Full HTTP stack tests
└── Behat/                                 # E2E business scenarios
```
...
| **design-patterns** | design pattern, factory, specification, strategy, state machine, which pattern | When/how to apply Factory, Specification, Strategy, State, Null Object, Decorator, Template Method, Adapter |
| **solid-analysis** | SOLID, SRP, OCP, God class, type switch, fat interface, code smell | Detects SOLID violations with severity + remediation |
| **leaky-abstractions** | leaky abstraction, framework leakage, Doctrine in domain, port leak, exception translation | Finds framework/infra leaking through hexagonal boundaries |
| **bounded-contexts** | bounded context, cross-context, shared kernel, anti-corruption layer, ubiquitous language | Detects cross-context coupling, draws context map |
...
| **feature-builder** | Opus | Builds a complete vertical feature slice (Domain + Application + Infrastructure + tests) from a plain-language description. Applies SOLID/GRASP, picks design patterns by need. Write-capable. |
...
> Build a checkout feature with order placement and payment
```

The `hexagonal-architect` and `hexagonal-reviewer` agents are **read-only** — they analyze and report. The `feature-builder` agent **writes code**: it generates the full slice for a described feature.

---

## Commands

Unlike skills (auto-triggered) and agents (invoked by context), commands are explicit slash commands you type.

| Command | Purpose |
|---------|---------|
| `/audit-branch [base] [-- focus]` | Audits the current branch against **every** project skill (hexagonal rules, SOLID, leaky abstractions, bounded contexts, CQRS, validation, security, persistence, patterns, tests). Produces one consolidated CRITICAL/WARNING/INFO report with a verdict. Read-only. |
| `/audit-tests [base] [-- focus]` | Audits the **test pyramid**: counts unit / integration / functional / Behat E2E tests, checks the shape (many unit, fewer integration, fewest E2E — flags inverted pyramid / ice-cream cone), verifies coverage of changed Domain/Application code, and flags double misuse (mocking internals instead of ports). Read-only. |
...
> /audit-tests                     # audit the test pyramid of the current branch
> /audit-tests develop -- focus on coverage of the Order module
```
